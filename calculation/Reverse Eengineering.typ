...
#thm("局域调制算符 $U_A$ 的解析表达与基底符号修正")[
  ...
  *关键符号修正（Phase Correction）*：深入剖析原论文基底可发现，对于 $K=3$ 的展开态：
  $
...
+ *多级受控旋转 (Uniformly Controlled Rotations)*：将第 $n$ 个量子比特作为唯一的局域目标位，前 $n-1$ 个比特作为控制位。这是一个由 $2^{n-1}$ 个条件分支组成的庞大受控门阵列。当且仅当控制寄存器处于特定基态 $ket(K)$ 时，在第 $n$ 个比特上触发对应的 $R_y(2theta_(K+1))$ 旋转。
